...
page_fault (struct intr_frame *f) 
{
  ...
  if(!user) { 
    ...
    if (!not_present) {
        ...
        f->eax = 0xffffffff;
        fs_give();      // unlock lock if it's occupied by read/write
        thread_exit();
    } else {
  ...
  if(not_present) {
    ...
    if(spg == NULL) {
      //printf(" Location -> nowhere  ADDRESS %p \n", va);
    ...
    if(spg->location & MMF) {
      //printf("Location -> mmf ADDRESS  \n");
      if(load_page_mmf(spg, va)) return;
      else PANIC("Could not load_page in page_fault!");
    }

    /* A page exists in the supp page table. Might be in SWAP space */
    if(spg->location & SWAP) {
       //printf( "Location -> swap ADDRESS %p \n", va);
       if(load_page_swap(spg,va)) return;
       else PANIC("Coudl not load_page in page_fault!");
    }

    /* A page exists in the supp page table. Might be on DISK. */
    if(spg->location & DISK) {
      //printf(" Location -> disk  ADDRESS %p \n", va);
      if(load_page_disk(spg, va)) return;
      else PANIC("Could not load_page in page_fault!");
...
#ifdef VM
bool load_page_disk(struct sup_page* spg, uint8_t* va)
{
...
bool 
load_page_mmf(struct sup_page *spg, uint8_t *va)
{
    ASSERT(spg->location & MMF);
    struct file *pfile  = spg->mmf_file;
...
bool 
load_page_swap(struct sup_page *spg, uint8_t *va)
{ 
    ASSERT(spg->location & SWAP);
    struct file* pfile = thread_current()->execfile;

    uint8_t *kpage = frame_map(va, PAL_USER);

    /* Maybe no free space left -> must be handled in frame_free */
    if (!swap_in(spg->swap_index, kpage) || !install_page (va, kpage, spg->writable)) {
       frame_free(kpage);
       return false;
    } 
    
    swap_free(spg->swap_index);
    spg->location = MEMORY;
    spg->frame_no = ADDR_TO_PFNO(kpage);
    return true;
}
...
bool 
load_page(struct sup_page *spg, uint8_t *va)
{
    if (spg->location & DISK)
        return load_page_disk(spg, va);
    if (spg->location & SWAP)
        return load_page_swap(spg,va);
    if (spg->location & MMF)
        return load_page_mmf(spg,va);
