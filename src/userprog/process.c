...
start_process(void *load_p)
{
    ...
    /* If load failed, quit. Maybe we wan't this info?*/
    if (!success) {
        thread_current()->load_status = 1;
        sema_up(&thread_current()->loading_sema);
        thread_exit();
    } else {
        thread_current()->exec_file_name = exec;
...
{
    /* wait until the children with the correct tid releases the lock and return exit status */
    ...
    for (e = list_begin(&t->children); e != list_end(&t->children); e = list_next(e)) {
        ...
        /* wait until the semaphore is unlocked in process_exit() */
        if (child->tid == child_tid) {
                sema_down(&child->exiting_sema);
    ...
    /* checks if there's any children that has terminated before running exit
     * and destroys the thread descriptor */
    for (e = list_begin(&t->dying_children); e != list_end(&t->dying_children); e = list_next(e)) {
        struct thread *dying = list_entry(e, struct thread, dying_child);

        if (dying->tid == child_tid) {
            list_remove(e);
            palloc_free_page (dying);
            break;
        }
    }
    ...
    /* returns -1 if there's nothing to wait for or if theres no child with that pid */
    return status;
}
...
process_exit()
{
    ...
    /* make so parent knows that it terminated before and remove lock */
    if (cur->parent != NULL ) {
        sema_up(&cur->exiting_sema);
    }

    //TODO: Implement outcommented code if parent exits with waiting childs!
    /* if we have any children waiting for us, we must release their locks
    struct list_elem *e;
    ...
    */
    ...
    if (cur->exec_file_name == NULL) {
        printf("%s: exit(%d)\n", "no-such-file", cur->exit_status);
    } else {
        file_allow_write (cur->execfile);
        printf("%s: exit(%d)\n", cur->exec_file_name, cur->exit_status);
    }

#ifdef VM
    
    struct list_elem *e;
    for (e = list_begin(&cur->mappings);
         e != list_end(&cur->mappings); e = list_next(e)) {
    ...
    // saves thread exit status that is checked when parent wait from syscall exit(status)
}
