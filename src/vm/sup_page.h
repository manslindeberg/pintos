...
struct sup_page {
  ...
  /* MEMORY */
  unsigned frame_no; /* most sig 20 bits describing kernelel/phys addr */
  ...
  /* index location of address in swap space */
  size_t swap_index;

  struct file* mmf_file;
  /* We'll do this as a list, but a hash table could be better */
  /* A hash table is already provided in "lib/kernel/hash.h,.c"*/
  struct list_elem elem;
};

struct sup_page* new_file_sup_page(off_t, uint32_t, 
                       uint8_t*, bool);
struct sup_page* new_zero_sup_page(uint8_t*);
struct sup_page* new_mmf_sup_page(off_t , uint32_t , uint8_t* , bool);
struct sup_page* lookup_page(uint8_t*);
